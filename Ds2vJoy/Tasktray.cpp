...
void Tasktray::Init(HINSTANCE hInst, HWND hWnd)
{
	...
	wsprintf(m_nid.szTip, L"Ds2vJoy");
	CreateMenu();
}
...
void Tasktray::SwapMenuitem(int item)
{
	...
	case 0:
		if (tape.vJoyPaused)
			ModifyMenu(m_menu, MenuOffset + 4, MF_BYPOSITION | MF_POPUP, IDM_VJOY, _T("Start vJoy"));
		else
			ModifyMenu(m_menu, MenuOffset + 4, MF_BYPOSITION | MF_POPUP, IDM_VJOY, _T("Pause vJoy"));
		break;
	case 1:
		if (tape.ViGEmPaused)
...
void Tasktray::Message(WPARAM wParam, LPARAM lParam)
{
	...
	void Tasktray::GetMenuPosition(struct MenuPosition* menuposition)
	{
		...
		{
		case ABE_BOTTOM:
			*menuposition = { TPM_CENTERALIGN | TPM_BOTTOMALIGN, p.x, p.y -10 };
			break;
		case ABE_LEFT:
			*menuposition = { TPM_LEFTALIGN | TPM_BOTTOMALIGN, p.x +10, p.y };
			break;
		case ABE_RIGHT:
			*menuposition = { TPM_RIGHTALIGN | TPM_BOTTOMALIGN, p.x -10, p.y };
			break;
		case ABE_TOP:
			*menuposition = { TPM_CENTERALIGN | TPM_TOPALIGN, p.x, p.y +10 };
			break;
		}
	}
