...
INT_PTR LinksDlg::_proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SHOWWINDOW:
	{
		if (wParam == TRUE)
		{
			if (!firsttime)
			{
				HFONT hFont = CreateFont(12, 0, 0, 0, FW_MEDIUM, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, PROOF_QUALITY, DEFAULT_PITCH | FF_SWISS, L"Bahnschrift SemiLight SemiConde");
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP1NAME), WM_SETFONT, WPARAM(hFont), TRUE);
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP2NAME), WM_SETFONT, WPARAM(hFont), TRUE);
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP3NAME), WM_SETFONT, WPARAM(hFont), TRUE);
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP4NAME), WM_SETFONT, WPARAM(hFont), TRUE);
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP5NAME), WM_SETFONT, WPARAM(hFont), TRUE);
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP1LOC), WM_SETFONT, WPARAM(hFont), TRUE);
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP2LOC), WM_SETFONT, WPARAM(hFont), TRUE);
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP3LOC), WM_SETFONT, WPARAM(hFont), TRUE);
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP4LOC), WM_SETFONT, WPARAM(hFont), TRUE);
				SendMessage(GetDlgItem(hWnd, IDC_LINKS_APP5LOC), WM_SETFONT, WPARAM(hFont), TRUE);
				firsttime = true;
			}
