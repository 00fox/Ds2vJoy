...
void vJoyButton::setOverwrite(bool isover)
{
	...
	case axis_XY:
		overwrite[axis_X] = isover;
		overwrite[axis_Y] = isover;
		m_overwrite = isover;
		break;
	case axis_ZRZ:
		overwrite[axis_Z] = isover;
		overwrite[axis_RZ] = isover;
		m_overwrite = isover;
		break;
	case axis_RXRY:
		overwrite[axis_RX] = isover;
		overwrite[axis_RY] = isover;
		m_overwrite = isover;
		break;
	case axis_SL0SL1:
		overwrite[axis_SL0] = isover;
		overwrite[axis_SL1] = isover;
		m_overwrite = isover;
		break;
	default:
		m_overwrite = isover;
...
int vJoyButton::GetScrollVal()
{
	...
	case Type_DPad:
		return 10;
	case Type_Ring:
		return (((*m_data - 16384) * (*m_data - 16384)) + ((*m_data2 - 16384) * (*m_data2 - 16384))) / 2236962;
	default:
		return 0;
	}
}
