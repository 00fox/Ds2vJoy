...
{
	if (vigem_target_x360_registered || vigem_target_ds4_registered)
		return;
	if (tape.ViGEmPaused)
		return;
...
//	  vigem_target_set_pid(pad, unsigned short pid);
		...
		if (!VIGEM_SUCCESS(err))
		{
			echo(L"ViGEm Callback Registration failed with error: %s", ViGEmError(err));
			return;
		}
		else
			vigem_target_x360_registered = true;
	...
	case 2: //DS4
	{
		...
		err = vigem_target_ds4_register_notification(client, pad, &Notify_DS4, nullptr);
		if (!VIGEM_SUCCESS(err))
		{
			echo(L"ViGEm Callback Registration failed with error: %s", ViGEmError(err));
			return;
			...
			vigem_target_ds4_registered = true;
...
{
	if (vigem_target_x360_registered) { vigem_target_x360_unregister_notification(pad); vigem_target_x360_registered = false; }
	if (vigem_target_ds4_registered) { vigem_target_ds4_unregister_notification(pad); vigem_target_ds4_registered = false; }
...
void ViGEm::Run()
{
	...
	case 1: //Xbox360
	{
		...
		if ((tape.target_X360[2]) ? ((m_vj[2]) ? (m_vj[2]->isPushed()) : false) : (m_ds[2]) ? (m_ds[2]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_DPAD_UP; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_DPAD_UP)); }						//^
		if ((tape.target_X360[3]) ? ((m_vj[3]) ? (m_vj[3]->isPushed()) : false) : (m_ds[3]) ? (m_ds[3]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_DPAD_RIGHT; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_DPAD_RIGHT)); }				//>
		if ((tape.target_X360[4]) ? ((m_vj[4]) ? (m_vj[4]->isPushed()) : false) : (m_ds[4]) ? (m_ds[4]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_DPAD_DOWN; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_DPAD_DOWN)); }					//v
		if ((tape.target_X360[5]) ? ((m_vj[5]) ? (m_vj[5]->isPushed()) : false) : (m_ds[5]) ? (m_ds[5]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_BACK; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_BACK)); }							//7
		if ((tape.target_X360[6]) ? ((m_vj[6]) ? (m_vj[6]->isPushed()) : false) : (m_ds[6]) ? (m_ds[6]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_START; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_START)); }							//8
		if ((tape.target_X360[7]) ? ((m_vj[7]) ? (m_vj[7]->isPushed()) : false) : (m_ds[7]) ? (m_ds[7]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_X; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_X)); }									//3
		if ((tape.target_X360[8]) ? ((m_vj[8]) ? (m_vj[8]->isPushed()) : false) : (m_ds[8]) ? (m_ds[8]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_Y; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_Y)); }									//4
		if ((tape.target_X360[9]) ? ((m_vj[9]) ? (m_vj[9]->isPushed()) : false) : (m_ds[9]) ? (m_ds[9]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_B; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_B)); }									//2
		if ((tape.target_X360[10]) ? ((m_vj[10]) ? (m_vj[10]->isPushed()) : false) : (m_ds[10]) ? (m_ds[10]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_A; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_A)); }							//1
		if ((tape.target_X360[11]) ? ((m_vj[11]) ? (m_vj[11]->isPushed()) : false) : (m_ds[11]) ? (m_ds[11]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_RIGHT_THUMB; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_RIGHT_THUMB)); }		//10
		if ((tape.target_X360[12]) ? ((m_vj[12]) ? (m_vj[12]->isPushed()) : false) : (m_ds[12]) ? (m_ds[12]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_GUIDE; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_GUIDE)); }					//No
		if ((tape.target_X360[14]) ? ((m_vj[14]) ? (m_vj[14]->isPushed()) : false) : (m_ds[14]) ? (m_ds[14]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_LEFT_SHOULDER; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_LEFT_SHOULDER)); }	//5
		if ((tape.target_X360[15]) ? ((m_vj[15]) ? (m_vj[15]->isPushed()) : false) : (m_ds[15]) ? (m_ds[15]->isPushed()) : false) { wButtons = wButtons | XUSB_GAMEPAD_RIGHT_SHOULDER; head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_RIGHT_SHOULDER)); }	//6
		...
		if (report_X360.bRightTrigger) { head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_RIGHT_TRIGGER)); }
		if (report_X360.sThumbLX) { head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_LEFT_X)); }
		if (report_X360.sThumbLY) { head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_LEFT_Y)); }
		if (report_X360.sThumbRX) { head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_RIGHT_X)); }
		if (report_X360.sThumbRY) { head += wsprintf(head, L"%s ", X360_String(XUSB_GAMEPAD_RIGHT_Y)); }
	...
	case 2: //DS4
	{
		...
		if ((tape.target_DS4[2]) ? ((m_vj[2]) ? (m_vj[2]->isPushed()) : false) : (m_ds[2]) ? (m_ds[2]->isPushed()) : false) { dButtons = dButtons | 0x02; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_NORTH)); }											//^
		if ((tape.target_DS4[3]) ? ((m_vj[3]) ? (m_vj[3]->isPushed()) : false) : (m_ds[3]) ? (m_ds[3]->isPushed()) : false) { dButtons = dButtons | 0x04; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_EAST)); }											//>
		if ((tape.target_DS4[4]) ? ((m_vj[4]) ? (m_vj[4]->isPushed()) : false) : (m_ds[4]) ? (m_ds[4]->isPushed()) : false) { dButtons = dButtons | 0x08; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_SOUTH)); }											//v
		if ((tape.target_DS4[5]) ? ((m_vj[5]) ? (m_vj[5]->isPushed()) : false) : (m_ds[5]) ? (m_ds[5]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_SHARE; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_SHARE)); }								//9
		if ((tape.target_DS4[6]) ? ((m_vj[6]) ? (m_vj[6]->isPushed()) : false) : (m_ds[6]) ? (m_ds[6]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_OPTIONS; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_OPTIONS)); }							//10
		if ((tape.target_DS4[7]) ? ((m_vj[7]) ? (m_vj[7]->isPushed()) : false) : (m_ds[7]) ? (m_ds[7]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_SQUARE; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_SQUARE)); }								//1
		if ((tape.target_DS4[8]) ? ((m_vj[8]) ? (m_vj[8]->isPushed()) : false) : (m_ds[8]) ? (m_ds[8]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_TRIANGLE; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_TRIANGLE)); }							//4
		if ((tape.target_DS4[9]) ? ((m_vj[9]) ? (m_vj[9]->isPushed()) : false) : (m_ds[9]) ? (m_ds[9]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_CIRCLE; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_CIRCLE)); }								//3
		if ((tape.target_DS4[10]) ? ((m_vj[10]) ? (m_vj[10]->isPushed()) : false) : (m_ds[10]) ? (m_ds[10]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_CROSS; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_CROSS)); }							//2
		if ((tape.target_DS4[11]) ? ((m_vj[11]) ? (m_vj[11]->isPushed()) : false) : (m_ds[11]) ? (m_ds[11]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_THUMB_RIGHT; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_THUMB_RIGHT)); }				//12
		if ((tape.target_DS4[12]) ? ((m_vj[12]) ? (m_vj[12]->isPushed()) : false) : (m_ds[12]) ? (m_ds[12]->isPushed()) : false) { sButtons = sButtons | DS4_SPECIAL_BUTTON_PS; head += wsprintf(head, L"%s ", DS4_String(DS4_SPECIAL_BUTTON_PS)); }				//13
		if ((tape.target_DS4[13]) ? ((m_vj[13]) ? (m_vj[13]->isPushed()) : false) : (m_ds[13]) ? (m_ds[13]->isPushed()) : false) { sButtons = sButtons | DS4_SPECIAL_BUTTON_TOUCHPAD; head += wsprintf(head, L"%s ", DS4_String(DS4_SPECIAL_BUTTON_TOUCHPAD)); }	//14
		if ((tape.target_DS4[14]) ? ((m_vj[14]) ? (m_vj[14]->isPushed()) : false) : (m_ds[14]) ? (m_ds[14]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_SHOULDER_LEFT; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_SHOULDER_LEFT)); }			//5
		if ((tape.target_DS4[15]) ? ((m_vj[15]) ? (m_vj[15]->isPushed()) : false) : (m_ds[15]) ? (m_ds[15]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_SHOULDER_RIGHT; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_SHOULDER_RIGHT)); }		//6
		if ((tape.target_DS4[16]) ? ((m_vj[16]) ? (m_vj[16]->isPushed()) : false) : (m_ds[16]) ? (m_ds[16]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_TRIGGER_LEFT; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_TRIGGER_LEFT)); }			//7
		if ((tape.target_DS4[17]) ? ((m_vj[17]) ? (m_vj[17]->isPushed()) : false) : (m_ds[17]) ? (m_ds[17]->isPushed()) : false) { wButtons = wButtons | DS4_BUTTON_TRIGGER_RIGHT; head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_TRIGGER_RIGHT)); }			//8
		...
		if (report_DS4.bTriggerR) { head += wsprintf(head, L"%s ", DS4_String(DS4_BUTTON_RIGHT_TR)); }
		if (report_DS4.bThumbLX != 127) { head += wsprintf(head, L"%s ", DS4_String(DS4_THUMB_LEFT_X)); }
		if (report_DS4.bThumbLY != 127) { head += wsprintf(head, L"%s ", DS4_String(DS4_THUMB_LEFT_Y)); }
		if (report_DS4.bThumbRX != 127) { head += wsprintf(head, L"%s ", DS4_String(DS4_THUMB_RIGHT_X)); }
		if (report_DS4.bThumbRY != 127) { head += wsprintf(head, L"%s ", DS4_String(DS4_THUMB_RIGHT_Y)); }
...
int ViGEm::ViGEmState(bool verbose)
{
	...
	if (verbose)
		...
		case 0:
			...
			break;
		case 1:
			echo(L"ViGEm driver is runing");
			break;
		case 2:
			echo(L"ViGEm driver is disabled");
			break;
		default:
			echo(L"ViGEm state unknown");
		}
...
BOOL ViGEm::ViGEmUninstall(bool verbose)
{
	...
	{
		{
			std::string devconpath = "Devcon.exe remove Nefarius\\ViGEmBus\\Gen1";
			LaunchCmd(devconpath.c_str());
		}
	...
	std::wstring installpath = PrfPathW() + L"\\" + ProgramFilesDirName + L"\\ViGEmBus\0\0";
...
BOOL ViGEm::ViGEmEnable(bool verbose)
{
	...
	std::string devconpath = "Devcon.exe enable Nefarius\\ViGEmBus\\Gen1";
	LaunchCmd(devconpath.c_str());
...
BOOL ViGEm::ViGEmDisable(bool verbose)
{
	...
	std::string devconpath = "Devcon.exe disable Nefarius\\ViGEmBus\\Gen1";
...
void CALLBACK ViGEm::Notify_X360(
	...
	LPVOID UserData)
{
	tape.FFBvJoyLeft = max(0, min(255, LargeMotor));
	tape.FFBvJoyRight = max(0, min(255, SmallMotor));

//	static int count = 1;
//	echo(L"%d - Large motor:%d - Small motor:%d", count++, (int)LargeMotor, (int)SmallMotor);
}

VOID CALLBACK ViGEm::Notify_DS4(
	PVIGEM_CLIENT Client,
	PVIGEM_TARGET Target,
	UCHAR LargeMotor,
	UCHAR SmallMotor,
...
//	static int count = 1;
//	echo(L"%d - Large motor:%d - Small motor:%d", count++, (int)LargeMotor, (int)SmallMotor);
//	echo(L"%d - Red:%d - Green:%d - Blue:%d", count++, LightbarColor.Red, LightbarColor.Green, LightbarColor.Blue);
}
